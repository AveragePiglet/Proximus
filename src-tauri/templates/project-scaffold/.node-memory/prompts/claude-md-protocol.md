...
Append this section to the project's existing CLAUDE.md during migration.
...
5. Run `python .node-memory/tools/validate.py` — must pass
...
- ⊥ adding nodes without at least one edge
- ⊤ every bug, invariant, decision, task gets a stable ID
- ⊤ node files ≤ 120 lines (enforced by validator)
